...
try:
    ...
    raise ImportError("A biblioteca 'pvlib' é obrigatória.")

# Presumindo que os imports externos continuam os mesmos:
import sys
import os
...
class DataCleaningTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, column_mapping=None, timezone='UTC', start_year=2018):
...
class SolarPositionTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, latitude, longitude, altitude=0, timezone='UTC'):
    ...
    def transform(self, X):
        ...
        df['am_abs'] = pvlib.atmosphere.get_absolute_airmass(rel_airmass, pressure=pvlib.atmosphere.alt2pres(self.altitude))
...
class ClearSkyTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, cs_model='esra', location=None):
    ...
    def transform(self, X):
        ...
        if self.cs_model == 'perez':
            cs = self.location.get_clearsky(df.index, model='ineichen', linke_turbidity=3)
            ...
            cs = ESRA(apparent_zenith=df['apparent_zenith'], dni_extra=df['dni_extra'], linke_turbidity=3.0, altitude=self.location.altitude)
            lista_dias_claros = cs.get_clear_days_pearson(df, threshold=0.95, min_ratio=0.8)
    ...
    def _is_clear(self, df):
        ...
        if freq is None:
            ...
            is_high_freq = median_diff <= pd.Timedelta(minutes=15)
        else:
        ...
        if is_high_freq:
            ...
            is_clear_window = pvlib.clearsky.detect_clearsky(ghi_series, cs_series, df.index, window_length=10, mean_diff=75, max_diff=75, lower_line_length=-5, upper_line_length=10, slope_dev=8)
    ...
    def _otimization_LT(self, df):
        ...
        for day in unique_days:
            ...
            if is_clear and len(sub_df) > 10: 
                res = minimize_scalar(self._objective_function, bounds=(1.0, 10.0), args=(sub_df,), method='bounded')
                if res.success:
                    optimal_tl = res.x
        ...
        cs_calibrated = pvlib.clearsky.ineichen(
            apparent_zenith=df['apparent_zenith'], airmass_absolute=df['am_abs'], linke_turbidity=df['Linke_Turbidity'], dni_extra=df['dni_extra'], perez_enhancement=True)
    ...
    def _objective_function(self, tl, sub_df):
        cs = pvlib.clearsky.ineichen(apparent_zenith=sub_df['apparent_zenith'], airmass_absolute=sub_df['am_abs'], linke_turbidity=tl, dni_extra=sub_df['dni_extra'], perez_enhancement=True)
        if sub_df['ghi'].isnull().all() or cs['ghi'].isnull().all():
            return 999999 
        return root_mean_squared_error(sub_df['ghi'], cs['ghi'])


class PhysicalFeaturesTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, nominal_power=156.0, auto_identify=True, target_col='power', kasten_corr=False):
        ...
        if self.auto_identify and self.target_col_internal == 'power':
            ...
            if all(col in df.columns for col in required):
                ...
                if len(df_fit) >= 50:
                    ...
                    try:
                        popt, _ = curve_fit(physical_power_model, X_data, Y_data, p0=[self.u0, self.u1], bounds=bounds, method='trf')
    ...
    def transform(self, X):
        ...
        df['kt'] = df['kt'].fillna(0).clip(0, 1.5)
...
class QualityControlTransformer(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        return self
    ...
    def transform(self, X):
        ...
        final_mask = (cond_alpha & cond_ghi_limits & cond_dhi_limits & cond_closure &
                      cond_dni_limits & cond_reject_ratio & cond_ghi_min_slope & cond_k & cond_diffuse_consistency)
        ...
        df['qc_reprovals'] = np.where(mask_5h, np.where(df['qc_reprovals'] == "", "min_5_hours", df['qc_reprovals'] + ";min_5_hours"), df['qc_reprovals'])
...
class FeatureEngineeringTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, target_col='power', nominal_power=156.0, start_year=2018, degradation_rate=0.05):
        self.target_col_internal = target_col
        self.nominal_power = nominal_power
        self.start_year = start_year
...
class ScalerTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        cols_x = [c for c in self.features_to_scale if c in X.columns]
        if cols_x:
            self.scaler.fit(X[cols_x])
        return self
    ...
    def transform(self, X):
        ...
        if cols_x:
            df[cols_x] = self.scaler.transform(df[cols_x])
        return df
